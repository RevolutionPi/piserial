...
extern int enc64(char*, uchar*, int);
...
MD5state* md5(uchar*, uint, uchar*, MD5state*);
 ...
 *  the last call
 */
MD5state *md5(uchar *p, uint len, uchar *digest, MD5state *s)
{
	uint a, b, c, d, tmp;
	uint i, done;
	Table *t;
...
int dec64(uchar *out, uchar *in, int n)
{
	ulong b24;
	uchar *start = out;
	int i, c;
